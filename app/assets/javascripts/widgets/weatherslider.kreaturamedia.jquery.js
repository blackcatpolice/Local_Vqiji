...
(function($) {
	...
	$.weatherSlider = function(el, options) {
		...
		ws.reInit = function(){
			...
			if( ws.o.enableSearchField ){
				...
				var hideSearch = function(){
					$(el).find('.ws-search-hidden').stop().animate({
						top : - $(el).find('.ws-searchbox').height() * 1.5
					}, 250, 'easeInOutQuart', function(){
						//$(el).find('.ws-focushelper').focus().remove().appendTo( $(el).find('.ws-infobox .ws-bg') );
						// FIXED：隐藏后页面跳动
						$(el).find('.ws-focushelper').remove().appendTo( $(el).find('.ws-infobox .ws-bg') );
	  ...
	  ws.getWeather = function(id,search){
	   ...
	   /*
	     翻译城市名 比如 成都 => Chengdu, CN
	   */
	    if (l.match(/[^\w\s,]/)) {
			  $.ajax("/widgets/weather/t.json", {
			    method: "GET",
			    dataType: "json",
			    data: {
			      search: l
      ...
      function _animateInfobox() {
		...
		ws.parseData = function(wData,id,lname){
			...
			var iData = '<h1'+dLocTim+'><span class="ws-loc" style="font-size: 22px !important;"'+dLoc+'>'+location+' </span></h1>'; /*<span class="ws-time ws-line ws-fline"'+dTime+'>更新时间：'+ hours+':'+minutes+' '+ampm+'</span>'*/
		...
		ws.animateIn = function(ob,what){
			...
			// Animating weather information box
			  ...
			  var height = ws.g.sliderWidth / 36 + 15;
				ob.css({
					height: height,
					top: height * 0.25
				});
	...
	$.weatherSlider.options = {
		...
		WWOAPIKey			: 'van3eenyz9wv7e8wx5rx8wam'
	};
