...
!function( $ ) {
	...
	var Datetimepicker = function(element, options) {
		...
		this.cssPosition = options.cssPositon || this.element.data('css-position') || "absolute";
		this.cssZIndex = options.cssZIndex || this.element.data('css-z-index');
	...
	Datetimepicker.prototype = {
		...
		place: function(){
			...
			/* FIXED： 使用 jquery.positon 重构 */
			var of = this.element, left = 0, 
			  at_horizontal = "left", at_vertical = "bottom",
			  my_horizontal = "left", my_vertical = "top";

      // 如果使用预定输入结构，以结构中的图标为对齐参考
			if (this.component) {
				if (this.pickerPosition.match(/^(bottom|top)-left$/i)) {
			...
			if (this.pickerPosition.match(/^top-(left|right)$/i)) {
				at_vertical = "top";
			  my_vertical = "bottom";
			}
			...
			this.picker.position({
			  ...
			  my: my_horizontal + (left >=0 ? ("+" + left) : left) + " " + my_vertical,
			  collision: "none"
			}).css({
				zIndex: (this.cssZIndex != null) ? this.cssZIndex : (parseInt(this.element.parents().filter(function() {
